	...
	<div class="row mb-2">
		...
		<form name="frm" class="col-8 col-md-5">
			<div class="input-group">
				<input name="query" class="form-control" value="엔시티">
...
<script id="temp_shop" type="x-handlebars-template">
	<table class="table table-borderd table-hover">
		<tr class="text-center">
			...
			<td>아이디</td><td colspan=2>상품명</td><td>가격</td><td>저장</td>
		</tr>
		{{#each items}}
		<tr gid="{{productId}}" img="{{image}}" title="{{title}}" brand="{{brand}}" price="{{lprice}}">
			...
			<td>{{lprice}}</td>
			<td><button class="btn btn-outline-success insert">저장</button></td>
		</tr>
		{{/each}}
	</table>
</script>
