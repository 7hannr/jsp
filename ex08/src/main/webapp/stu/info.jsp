...
<div>
	...
	<div class="input-group mb-2">
		<div id="div_cou"></div>
		<button class="btn btn-outline-danger" id="insert">수강신청</button>
	</div>
...
<script id="temp_enroll" type="x-handlebars-template">
	<table class="table table-bordered table-hover">
		...
		<tr class="text-center">
			...
			<td>{{pname}}({{pcode}})</td>
			<td>{{edate}}</td>
			<td><button class="btn btn-outline-danger btn-sm delete" lcode="{{lcode}}">취소</button></td>
		</tr>
		{{/each}}
	</table>
</script>
<script id="temp_cou" type="x-handlebars-template">
   <select class="form-select" id="lcode">
      {{#each .}}
            <option value="{{lcode}}">-&nbsp;{{lname}} ({{lcode}})&nbsp;({{pname}})&nbsp;-&nbsp;&nbsp;({{persons}}/{{capacity}})</option>
      {{/each}}
   </select>
</script>
<script>
